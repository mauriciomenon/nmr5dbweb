...
make_server = getattr(pytest.importorskip("werkzeug.serving"), "make_server")
duckdb = pytest.importorskip("duckdb")
sync_playwright = getattr(pytest.importorskip("playwright.sync_api"), "sync_playwright")
try:
...
def test_success_frontend_smoke_compare_pagination_and_export(ui_server, sample_data, tmp_path):
    with with_browser() as (_browser, _browser_context, page):
        ...
        assert isinstance(payload["summary"]["recommended_actions"], list)
        assert payload["summary"]["recommended_actions"]
